...
ValidationLayers::ValidationLayers(std::vector<const char*> layerNames)
    : validationLayers_(std::move(layerNames)) {  }
...
void ValidationLayers::clean(VkInstance& instance) {
...
uint32_t ValidationLayers::getValidationLayersCount() {
    return static_cast<uint32_t>(validationLayers_.size());
}

const char **ValidationLayers::getValidationLayers() {
    return validationLayers_.data();
}
...
void ValidationLayers::DestroyDebugUtilsMessengerEXT(VkInstance instance,
